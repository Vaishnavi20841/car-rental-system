...
export const createBooking = async (req, res) => {
  ...
  try {
    ...
    if (!customer || !email || !car || !pickupDate || !returnDate) {
      ...
      return res.status(400).json({ success: false, message: "Missing required fields (customer, email, car, pickupDate, returnDate)" });
    }
    ...
    if (Number.isNaN(pickup.getTime()) || Number.isNaN(ret.getTime()) || pickup > ret) {
      await session.abortTransaction(); session.endSession();
      return res.status(400).json({ success: false, message: "Invalid pickup/return dates" });
    }
    ...
    if (overlappingCount > 0) {
      await session.abortTransaction(); session.endSession();
      return res.status(409).json({ success: false, message: "Car already booked for the selected dates" });
    }
...
export const updateBooking = async (req, res, next) => {
  try {
    ...
    const updated = await booking.save(); // post save syncs Car.bookings
    res.json(updated);
  } catch (err) { next(err); }
};
